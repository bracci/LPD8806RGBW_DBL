...
LPD8806RGBW_DBL::LPD8806RGBW_DBL(uint16_t n) {
  pixels = NULL;
  begun  = false;
  updateLength(n);
  updatePins();
...
LPD8806RGBW_DBL::LPD8806RGBW_DBL(uint16_t n, uint8_t dpin, uint8_t cpin) {
  pixels = NULL;
  begun  = false;
  updateLength(n);
...
void LPD8806RGBW_DBL::begin(void) {
...
void LPD8806RGBW_DBL::updatePins(void) {
...
void LPD8806RGBW_DBL::updatePins(uint8_t dpin, uint8_t cpin) {
...
void LPD8806RGBW_DBL::startSPI(void) {
#ifdef __AVR_ATtiny85__
  ...
  // SPI bus is run at 2MHz.  Although the LPD8806RGBW_DBL should, in theory,
...
void LPD8806RGBW_DBL::startBitbang() {
...
uint16_t LPD8806RGBW_DBL::numPixels(void) {
  return numLEDs;
}
...
void LPD8806RGBW_DBL::writeOut(uint8_t colorContent){
	if(hardwareSPI){
...
uint32_t LPD8806RGBW_DBL::Color(byte r, byte g, byte b) {
...
void LPD8806RGBW_DBL::setPixelColor(uint16_t n, uint8_t r, uint8_t g, uint8_t b) {
...
void LPD8806RGBW_DBL::setPixelColor(uint16_t n, uint32_t c) {
  if(n < numLEDs) { // Arrays are 0-indexed, thus NOT '<='
    uint8_t *p = &pixels[n * 3];
    *p++ = (c >> 16) | 0x80;
    *p++ = (c >>  8) | 0x80;
...
uint32_t LPD8806RGBW_DBL::getPixelColor(uint16_t n) {
  if(n < numLEDs) {
    uint16_t ofs = n * 3;
